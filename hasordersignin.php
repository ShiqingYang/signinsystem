...
if ($stunum) {
    $sqlorder = "select * from sign_order where stunum='" . $stunum . "'";
    $queryorder = mysql_query($sqlorder);
    if ($rsorder = mysql_fetch_array($queryorder)) {
        //echo $stunum;
        ...
        if ($resultStuinfo = mysql_fetch_array($queryStuinfo)) {
            ...
            $sqlfinish = "update sign_order set isfinish=1 where stunum='" . $stunum . "'";
            $queryf = mysql_query($sqlfinish);
            $rsf = mysql_fetch_array($queryf);
            $sqlInsert = "insert into sign_record(sid,stunum,isorder) values ('" . $sid . "','" . $stunum . "',1)";
            //echo $sqlInsert;
        ...
        echo "<script>Alert('此学号未预定，请稍后入场！');</script>";
        echo "<script>play_click('audio/fail.wav');</script>";
    }
...
<div class="hasorder">
    <form name="formstunum" id="formstunum" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
        <span class="signfont">已订票同学拍卡：&nbsp;</span>
